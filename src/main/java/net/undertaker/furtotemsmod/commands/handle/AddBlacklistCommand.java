...
public class AddBlacklistCommand {
    public static LiteralArgumentBuilder<CommandSourceStack> register() {
        return Commands.literal("add_blacklist")
                .then(
                        Commands.argument("player",  GameProfileArgument.gameProfile())
                                ...
                                .executes(AddBlacklistCommand::executeAdd));
    }
    ...
    private static int executeAdd(CommandContext<CommandSourceStack> context) throws CommandSyntaxException{
        ...
        if(data.isBlacklisted(player.getUUID(),targetUUID)){
            source.sendFailure(Component.translatable( "command.add_blacklist.failure.exist", targetProfile.getName()));
            return 0;
        }

        data.addToBlacklist(player.getUUID(), targetUUID);

        SyncWhitelistPacket packet = new SyncWhitelistPacket(data.getWhitelistPlayers());
        ModNetworking.sendToPlayer(packet, player);
    source.sendSuccess(() ->
            Component.translatable("command.add_blacklist.success", targetProfile.getName()), true);
        return 1;
    }
}
